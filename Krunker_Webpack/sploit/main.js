			...
			if(this.config.game.error_tips){
				if(has('connection banned')){
					if(this.config.game.proxy)actions.alert(
						...
						actions.alert(
							`<p>You got IP Banned which means the Tool can not Bypass this art of ban.\nOptions to get Unbanned are:\n - Searching for a Krunker UNDETECTED VPN which can change yours IP\n - Waiting until the next Updates, you normally get unbanned after some Time  </p>
							<ul>
							</ul>`
						);
					}
					...
					`<p>You were banned from the Krunker Anti Cheat.\nYou have been logged out of the current Account to Bypass the ban.</p>
					<p>Please do not sign back to the Banned Account because you will get Banned again.</p>`
				).then(() => location.assign('/'));
			}
